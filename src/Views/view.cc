...
s21::View::View(QWidget* parent) : QWidget(parent), ui(new Ui::View) {
  ui->setupUi(this);
}
...
void s21::View::SetVertices(std::vector<double>* vertices) {
  ...
  ui->label_count_vertex->setText("Vertices: " +
                                  QString::number(vertices->size() / 3));
}
...
void s21::View::on_pushButton_rotate_clicked() {
  emit ControllerRotateModel(ui->doubleSpinBox_x_rotate->value(),
                             ui->doubleSpinBox_y_rotate->value(),
                             ui->doubleSpinBox_z_roate->value());
}

void s21::View::on_pushButton_move_clicked() {
  emit ControllerMoveModel(ui->doubleSpinBox_x_move->value(),
...
void s21::View::on_pushButton_redraw_clicked() {
  ui->widget->GetState()->setValue("vertices_size",
                                   ui->spinBox_size_ver->value());
  ui->widget->GetState()->setValue("lines_size",
                                   ui->spinBox_size_line->value());

  ui->widget->GetState()->setValue("vertices_type",
                                   ui->comboBox_type_ver->currentIndex());
  ui->widget->GetState()->setValue("lines_type",
                                   ui->comboBox_type_line->currentIndex());

  ui->widget->GetState()->setValue(
      "projection_type", ui->comboBox_type_projection->currentIndex());
  ui->widget->update();
}
