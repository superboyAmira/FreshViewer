...
/*!
    ...
    ModelScene выступает в роли фасада для большого набора классов реализующих
   парсинг и хранения данных их .obj файлов. Клиентом выступает Controller.
*/
class ModelScene {
 public:
  ...
  /*!
     ...
     std::invalid_argument, при ошибки выделения памяти вызывает исключение
     std::bad_alloc.
  */
  ...
  /*!
      ...
      \param y
      \param z
      \details Изменяет напрямую координаты вершин, никак не взаимодействут с
     движком представления.
  */
  ...
  */
  ...
  Scene* GetScene() const noexcept { return scene_; };

  /*!
      \brief Геттер для формирования std::vector вершин, пригодных для обработки
     OpenGL.
  */
  std::vector<double>* GetVertices() {
    vertices_.clear();
    for (auto it_figure = scene_->GetFigures().begin();
         it_figure != scene_->GetFigures().end(); ++it_figure) {
      for (auto it_vertex = it_figure.base()->GetVertices().begin();
           it_vertex != it_figure.base()->GetVertices().end(); ++it_vertex) {
  ...
  /*!
      \brief Геттер для формирования std::vector индексов вершин (линий),
     пригодных для обработки OpenGL.
  */
  std::vector<int>* GetLines() {
    lines_.clear();
    for (auto it_figure = scene_->GetFigures().begin();
         it_figure != scene_->GetFigures().end(); ++it_figure) {
      for (auto it_edges = it_figure.base()->GetEdges().begin();
           it_edges != it_figure.base()->GetEdges().end(); ++it_edges) {
