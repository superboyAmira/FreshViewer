...
/*!
    ...
    для производных классов MoveTransformation, RotateTransformation,
   ScaleTransformation.
*/
...
class MoveTransformation : public BaseTransormation {
  ...
  */
  void TransformSceneX(double x, Scene& scene) override {
    for (auto it = scene.GetFigures().at(0).GetVertices().begin();
         it != scene.GetFigures().at(0).GetVertices().end(); ++it) {
